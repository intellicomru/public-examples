...
our $cnt=0;
## заполняем масив случанйми цифрами 
...
@big_massiv =(); ## чистим память от старого массива. 
#print join(",",@big_mass)."\n";

my $dig=6463;  ## выбираем число для поиска 

my $StartDi=[Time::HiRes::gettimeofday()];    # Точное время старта поиска
my $index;
...
## находим нужный элемент. заодно замеряем нагрузку 
   $index=get_index($dig,\@big_mass,0,$#big_mass); 
}
print "RESULT: $dig near ".$big_mass[$index]."  [index = $index]  \n";
